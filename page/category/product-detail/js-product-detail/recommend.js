...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ... (Các hàm toggleSuggestBox và addSuggestToCart ở đây)
    ...
    // 5. HÀM XỬ LÝ: THU NHỎ KHI CLICK BÊN NGOÀI (ĐOẠN MỚI THÊM)
    document.addEventListener('click', function(event) {
        const box = document.getElementById('suggest-box');
        
        // Đảm bảo cửa sổ đang mở và không chứa phần tử được click
        if (box && !box.contains(event.target) && !box.classList.contains('collapsed')) {
            // Thu nhỏ
...
}); // Kết thúc document.addEventListener('DOMContentLoaded', function() { ... })
